...
struct mcmf {
  ...
  void calc() {          // calc flow & cost
    vector<int> d(V), p(V), vis(V), q(V << 2);
    while (true) {
      fill(d.begin(), d.end(), inf);
      fill(p.begin(), p.end(), -1);
      d[S] = 0;                            // SPFA (Shortest Path Faster Algorithm)
      q[0] = S;                            // https://en.wikipedia.org/wiki/Shortest_Path_Faster_Algorithm
      int t = 1; ll af = INF;
      ...
      if (d[T] == inf) break;              // shortest path doesn't exist
